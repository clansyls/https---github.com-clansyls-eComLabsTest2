...
function fn_normalized_shipping_rate(&$normalized_data, $rates, $rate_type)
{
    foreach ($rates as $rate) {
        ...
        if ($rate['range_to_value'] !== '') {
            $rate['range_to_value'] = $rate_type === 'I' ? (int) $rate['range_to_value'] : (float) $rate['range_to_value'];
        }
        $rate['range_from_value'] = $rate_type === 'I' ? (int) $rate['range_from_value'] : (float) $rate['range_from_value'];
        $rate['value'] = fn_format_price($rate['value']);
        $rate['per_unit'] = empty($rate['per_unit']) ? YesNo::NO : $rate['per_unit'];

        if (!isset($normalized_data[$rate_type][$rate['range_from_value']]) || (float) $normalized_data[$rate_type][$rate['range_from_value']]['value'] == 0) {
            $normalized_data[$rate_type]["$rate[range_from_value]"] = [
...
function fn_calculate_cart_content(
    ...
    if (is_array($cart['products'])) {
        ...
        if ($calculate_shipping === 'S' && !Registry::isExist($shipping_cache_key)) {
            $calculate_shipping = 'A';
        }
...
function fn_get_available_shippings($company_id = null, $get_service_params = false, $storefront_id = null)
{
    ...
    if ($company_id) {
        /** @var \Tygh\Storefront\Repository $repository */
        $repository = Tygh::$app['storefront.repository'];
        $storefront_ids = empty($storefront_id) ? [] : (array) $storefront_id;
        if (!$storefront_ids) {
